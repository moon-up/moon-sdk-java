...
public class Message {
  ...
  private List<FiatCurrency> fiat = new ArrayList<>();

  public static final String SERIALIZED_NAME_CRYPTO = "crypto";
  @SerializedName(SERIALIZED_NAME_CRYPTO)
  private List<CryptoCurrency> crypto = new ArrayList<>();

  public Message() {
  }

  public Message fiat(List<FiatCurrency> fiat) {
    
    this.fiat = fiat;
    return this;
  }

  public Message addFiatItem(FiatCurrency fiatItem) {
    if (this.fiat == null) {
      this.fiat = new ArrayList<>();
    }
    this.fiat.add(fiatItem);
    return this;
  }
  ...
  **/
  ...
  public List<FiatCurrency> getFiat() {
    return fiat;
  }


  public void setFiat(List<FiatCurrency> fiat) {
    this.fiat = fiat;
  }


  public Message crypto(List<CryptoCurrency> crypto) {
    
    this.crypto = crypto;
    return this;
  }

  public Message addCryptoItem(CryptoCurrency cryptoItem) {
    if (this.crypto == null) {
  ...
  **/
  ...
  public boolean equals(Object o) {
    ...
    return Objects.equals(this.fiat, message.fiat) &&
        Objects.equals(this.crypto, message.crypto);
  }
  ...
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      ...
      if (!jsonObj.get("fiat").isJsonArray()) {
        throw new IllegalArgumentException(String.format("Expected the field `fiat` to be an array in the JSON string but got `%s`", jsonObj.get("fiat").toString()));
      }

      JsonArray jsonArrayfiat = jsonObj.getAsJsonArray("fiat");
      // validate the required field `fiat` (array)
      for (int i = 0; i < jsonArrayfiat.size(); i++) {
        FiatCurrency.validateJsonElement(jsonArrayfiat.get(i));
      };
      // ensure the json data is an array
      if (!jsonObj.get("crypto").isJsonArray()) {
      ...
      JsonArray jsonArraycrypto = jsonObj.getAsJsonArray("crypto");
      // validate the required field `crypto` (array)
      for (int i = 0; i < jsonArraycrypto.size(); i++) {
        CryptoCurrency.validateJsonElement(jsonArraycrypto.get(i));
      };
  }
